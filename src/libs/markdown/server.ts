...
export async function getPostBySlug(slug: string): Promise<Post | null> {
  try {
    ...
    // 处理图片链接
    const { content: processedImageContent } =
      imageProcessor.processImages(content)
    ...
    const post: Post = {
      ...
      createdTime: format(stats.birthtime, "yyyy-MM-dd HH:mm:ss", {
        locale: zhCN,
      }),
...
// 根据分类过滤文章 - Deprecated
export async function getPostsByCategory(_category: string): Promise<Post[]> {
  return []
}
